...
export async function loginDeliveryPartner(email, password) { const data = await apiRequest('/auth/admin/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) }); if (data.user?.role !== 'DELIVERY_PARTNER') throw new Error('This account is not a delivery partner account'); await SecureStore.setItemAsync('auth_token', data.token); await SecureStore.setItemAsync('auth_user', JSON.stringify(data.user)); registerForPushNotifications(data.token, 'delivery').catch(() => undefined); return data; }
...
export async function verifyOtp(challengeToken, otp) { const data = await apiRequest('/auth/phone/verify-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ challengeToken, otp }) }); await SecureStore.setItemAsync('auth_token', data.token); await SecureStore.setItemAsync('auth_user', JSON.stringify(data.user)); registerForPushNotifications(data.token, 'delivery').catch(() => undefined); return data; }
